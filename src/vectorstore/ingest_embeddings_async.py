...
CHUNK_SIZE = 1000
BATCH_SIZE = 20
CONCURRENT_COMMENTS = 5  # Number of comments processed concurrently
...
async def chunk_data_for_comment(
    ...
    chunks = split_text_into_chunks(getattr(comment, "body", "") if not isinstance(comment.body, str) else comment.body)
    for chunk in chunks:
...
async def upsert_comment_chunks(qdrant: AsyncQdrantVectorStore, comment: Comment, issue: Issue) -> bool:
    ...
    if await comment_already_ingested(qdrant, int(issue.number), cast(int, comment.comment_id)):
...
async def process_issue_comments(qdrant: AsyncQdrantVectorStore, issue: Issue) -> None:
    session = get_session()  # Use async session if needed
